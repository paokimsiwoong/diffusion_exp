...
>>>> 데이터셋에서 특정한 국소패턴이 더 자주 등장하는 것(ex: CelebA셋의 경우 사람 피부 결, 머리카락이 연속되는 패턴 등의 미세 디테일)을 잘 배우지 못한 모델은 가능한 모든 국소 패턴의 확률을 같게 평가하게 되고 가능한 패턴이 매우 많기 때문에 그 결과로 예측하는 확률은 매우 작아진다. cross entropy는 실제 발생확률이 높은 사건(여기서는 패턴)을 모델이 발생확률이 낮다고 잘못 판단했을 때 더 큰 페널티를 준다.  
>>>
...
>
> ### Cross entropy
> 실제 발생확률이 높은 사건을 모델이 잘못 판단했을 때는 큰 페널티
>
