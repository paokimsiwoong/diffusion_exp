...
class DenoiseDiffusion:
    ...
    def p_sample(self, xt: torch.Tensor, t: torch.Tensor):
        ...
        # 상한
            # β_t는 정방향 과정(노이즈 추가 과정)에서 노이즈를 더할 때 사용한 q(x_t | x_{t-1})의 분산값
            # 원본 이미지 x_0를 모르는 상태에서(x_0가 어떤 사진 이미지인지 단순한 완전 노이즈인지 모름) == 불확실성 최대 
            # -> 역방향으로 돌아갈 때, 복원 최종 결과 x_0를 모르므로 최악의 경우(x_0가 N(0, I)에서 온 완전 노이즈인 경우)를 가정하면 
            # 노이즈를 추가하는 정방향의 최종 목적지 N(0, I)와 역방향의 최종 목적지가 동일하다.
            # => 정방향과 같은 크기의 노이즈를 그대로 사용
